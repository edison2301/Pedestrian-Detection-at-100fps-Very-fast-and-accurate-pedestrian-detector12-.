      ...
      _classes(classes)
{
    ...
    _pushUpBias =  Parameters::getParameter<double>("train.pushUpBias");
    _pushLeftBias =  Parameters::getParameter<double>("train.pushLeftBias");
    if (_pushUpBias > 1.0 || _pushUpBias < 0.0)
        throw std::invalid_argument("pushUpBias as to be in range [0,1]");

    if (_pushLeftBias > 1.0 || _pushLeftBias < 0.0)
        throw std::invalid_argument("pushLeftBias as to be in range [0,1]");
    if (_pushLeftBias > 0 && _pushUpBias > 0){
        throw std::invalid_argument("for now only pushing up OR pushing left allowed!");
    }
    _pushBias = std::max(_pushLeftBias, _pushUpBias);
    // nothing to do here
    return;
}
...
double WeakDiscreteTreeLearner::buildBalancedTree_real(
        const weights_t &weights)
{
    ...
    const size_t num_examples = _trainingData->getNumExamples();


    int ret = createNode_real(weights,
    ...
    int root_node_bottom_value = _rootd2[0]._feature.y + _rootd2[0]._feature.height;
    int root_node_left_value = _rootd2[0]._feature.x + _rootd2[0]._feature.width;



    const int leftResult = createNode_real(weights, _rootd2[0]._indices, 0, _rootd2[0]._splitIndex,
                                           _rootd2[1],root_node_bottom_value, root_node_left_value);


    const int rightResult = createNode_real(weights, _rootd2[0]._indices, _rootd2[0]._splitIndex, _rootd2[0]._indices.size(),
                                            _rootd2[2], root_node_bottom_value, root_node_left_value);

    // no Leafs found -> do not look for that node again
    if (leftResult == -1 && rightResult == -1)
    {
        throw std::runtime_error("ERROR: tree has no leave nodes");
    }

    if (leftResult != 0)
    {

        _rootd2[1] = TreeNode_v2(_rootd2[0]._threshold,_rootd2[0]._betaTrue, 0.5 ,_rootd2[0]._feature, _rootd2[0]._featureIndex);

    }

    if (rightResult != 0)
    {
        _rootd2[2] = TreeNode_v2(_rootd2[0]._threshold,0.5,_rootd2[0]._betaFalse ,_rootd2[0]._feature, _rootd2[0]._featureIndex);
    }
        ...
        const weights_t &weights)
{
    ...
    //createNode_slow(weights, indices, 0, _trainingData->getNumExamples(), _root, errorsum);
...
inline
int WeakDiscreteTreeLearner::getErrorEstimate_real(
        const weights_t &weights,
        const indices_t& indices, const size_t featureIndex,
        int num_bins , const int minv, const int maxv, double &error) const
{
...
int WeakDiscreteTreeLearner::createNode_real(
        const weights_t &weights,
        const indices_t &indices, const size_t start, const size_t end,
        TreeNode_v2 &node,
        const int root_node_bottom_height, const int root_node_left_width) const
{
...
#pragma omp parallel for reduction(+:return_value) schedule(guided)
    for (size_t featureIndex = 0; featureIndex < max_valid_feature_index; ++featureIndex)
    {
        if (_trainingData->getFeatureValidity(featureIndex)){
            ...
            return_value += getErrorEstimate_real(weights, indicesCrop, featureIndex, binsize, minv, maxv, error);
    ...
    for (size_t i = 0; i< minErrorsForSearch.size(); ++i){
        ...
        if (y < miny && _pushUpBias>0){
            miny = y;
            minFeatureIndex = this_feat_idx;

            //we want to have the training in the upper half of the window unconstrained
            if (miny < max((modelHeight/2), root_node_bottom_height + deltaH))
                break;
        }
        else if (x < minx && _pushLeftBias>0){
            minx = x;
            minFeatureIndex = this_feat_idx;

            //we want to have the training in the upper half of the window unconstrained
            if (minx < max((modelWidth/2), root_node_left_width + deltaH))
                break;
        }
    ...
    int minThreshold = -1;
    int alphaMin = -1;
    double minError = std::numeric_limits<int>::max();

    if (findThreshold_real(weights, indicesCrop, minFeatureIndex, minError, minThreshold,
    ...
    double sumNeg=0;
    const FeaturesResponses &featuresResponses = _trainingData->getFeatureResponses();
    const FeaturesResponses::const_reference featureResponses = featuresResponses[minFeatureIndex];
    bool leftOfThreshold= true;
    double errorTrue = 0;
    double errorFalse = 0;

    //for real adaboost
    for (size_t i = 0; i < indicesCrop.size(); ++i)
    {
        const int index = indicesCrop[i];

        //FIXME CHECK this
        //if (weights[index]< 1E-12)
        //    continue;
        //std::cout << featureResponses[index] << " ";
        if ((featureResponses[index] >= minThreshold) && leftOfThreshold)
        {
            leftOfThreshold = false;
            errorTrue = sumPos/(sumPos+sumNeg);
            sumPos = 0;
            sumNeg = 0;
        }
    ...
    errorFalse =  sumPos/(sumPos+sumNeg);
    //handle boundaries
    if (errorFalse< 1e-12)
            errorFalse = 1e-12;
    if (errorFalse> 1-1e-12)
            errorFalse = 1-1e-12;
    if (errorTrue< 1e-12)
            errorTrue = 1e-12;
    if (errorTrue> 1-1e-12)
            errorTrue = 1-1e-12;

    double betaTrue = 0.5*log(errorTrue/(1-errorTrue));
    double betaFalse =0.5*log(errorFalse/(1-errorFalse));

    // set the shared_ptr to point towards a new node instance
    node = TreeNode_v2(minThreshold,betaTrue,betaFalse,
                           _trainingData->getFeature(minFeatureIndex),
                           minFeatureIndex, indicesCrop, splitIndexMin);
    return 0;
}
...
int WeakDiscreteTreeLearner::createNode_slow(
        const weights_t &weights,
        const indices_t& indices, const size_t start, const size_t end,
        TreeNode::shared_ptr &node, double &minError,
        const bool isLeft) const
{
    ...
    if(start > end)
    {
        throw std::invalid_argument("WeakDiscreteTreeLearner::createNode_slow received start > end but expected start <= end");
    }
    ...
    indices_t indicesmin, indicesCrop;
    int splitIndexMin = -1;
    //int minpos = -1;
    ...
    //indicesT holds the index to the data of this side of the tree

    //#pragma omp parallel for default(none) shared(doNotSplit, numFeatures, minerror, minFeat, minthr, minalpha, indicesmin, splitIndexMin, minpos) firstprivate(indicesCrop)
    for (size_t featureIndex = 0; featureIndex < numFeatures; ++featureIndex)
    {

        sortIndexesBasedOnDataPositions(indicesCrop, featureIndex);
        ...
        if (findThreshold(weights, indicesCrop, featureIndex,
                          error, threshold, alpha, splitIndex) == -1)
        {
#pragma omp critical
            {
                doNotSplit = true;
...
#pragma omp critical
        {
            if (error < minError)
            {
                ...
                minAlpha = alpha;
                indicesmin = indicesCrop;
                splitIndexMin = splitIndex;
            }
        }
    } // end of "for each feature"
    ...
    if(minFeatureIndex == -1)
    {
        throw std::runtime_error("Failed to find the feature with the lowest error");
    }

    node = TreeNode::shared_ptr(new TreeNode(minThreshold, minAlpha,
                                             _trainingData->getFeature(minFeatureIndex),
                                             minFeatureIndex,
                                             indicesmin, splitIndexMin, isLeft));
    return 0;
}
...
int WeakDiscreteTreeLearner::findThreshold_real(
        const weights_t &weights,
        const indices_t & sortedIndices,
        const size_t featureIndex,
        double &errorMin, int &thresholdMin, int &alphaMin, int &splitIndex) const
...
void boosted_learning::WeakDiscreteTreeLearner::setPushBias(double value)
{
    _pushBias = value;
